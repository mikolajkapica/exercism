...
(* so much better:
https://exercism.org/tracks/ocaml/exercises/grade-school/solutions/mizlan *)

let grade = Fn.flip Map.find_multi

let sorted = Map.map ~f: (List.sort ~compare: Poly.compare)

let roster school =
    sorted school |> Map.data |> List.concat
